...
export function computeStats(history: Contraction[]): Stats {
  ...
  if (count > 0) {
    ...
    // eslint-disable-next-line
    result.timeCoveredSecs = Math.round(
      (history[count - 1].endTime.getTime() - history[0].startTime.getTime()) / 1000,
    );
  }
  return result;
}
...
export class Controller511 {
  ...
  public render(includeSecondHand: boolean): void {
    ...
    let arrivalListStr = oneHourHistory.map(
      ...
      (deg) => `${deg.startDegrees},${(360 + deg.endDegrees - deg.startDegrees) % 360}`,
    ).reduce(
      (acc, s) => `${acc + s};`, '',
    );
...
export function attachController(
  ...
  view.startStopButton.addEventListener('click', () => {
  ...
  view.clearHistoryButton.addEventListener('click', () => {
